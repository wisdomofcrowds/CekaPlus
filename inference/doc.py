# -*- coding: utf-8 -*-
# Dependent One-Coin model for multi-label multi-class labeling
...
class DOCModel(model.Model):
    ...
    def initialize(self, dataset):
        ...
        for i in range(1, self.I + 1):
            ...
            doc_inst = DOCInstance(inst)
            doc_inst.initialize(self.M, self.K, self.R)
            self.instances.append(doc_inst)
        for w in range(1, self.J + 1):
            worker = dataset.get_worker(w)
            doc_worker = DOCWorker(worker)
            doc_worker.initialize(self.M, self.K, self.R)
            self.workers.append(doc_worker)
        for r in  range(1, self.R + 1):
            theta_list = [None]
            for m in range(1, self.M + 1):
    ...
    def infer(self, dataset, soft=True):
        ...
        print('DOC initial log-likelihood = ' + str(curr_likehihood))
        while ((count <= self.maxround) and (abs(curr_likehihood - last_likelihood) / abs(last_likelihood) > self.converge_rate)):
            ...
            curr_likehihood = self.loglikelihood()
            print('DOC round (' + str(count) + ') log-likelihood = ' + str(curr_likehihood))
            count += 1
